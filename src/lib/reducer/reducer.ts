...
function reducer(state: DataTableState, action: Action) {
  switch (action.type) {
    ...
    case ActionTypes.SET_PAGE_SIZE: {
      const totalPages = Math.ceil(state.filterResults / action.payload)
      return { ...state, pageSize: action.payload, totalPages }
    }
    case ActionTypes.SET_FILTER: {
      const filterResults = filterEntries({ entries: state.entries, filter: action.payload }).length
      const totalPages = Math.ceil(filterResults / state.pageSize)
