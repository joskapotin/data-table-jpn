 ...
 */
const isDate = (date: string) => !Number.isNaN(Date.parse(date))

/**
 * It sorts an array of objects by a given key and direction
 * If the key values are dates it will sort them by date
 */
const sortEntries = ({ entries, sortBy, sortDirection }: SortEntries) => {
 ...
 * string
 */
const filterEntries = (entries: Entries, filter: string) => {
